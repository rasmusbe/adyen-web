import AdyenCheckout, { PayPalFastlaneSDK } from '@adyen/adyen-web';
...
(async () => {
    const fastlane = new PayPalFastlaneSDK({
    ...
    await fastlane.initialize();
    console.timeEnd('Initialize');

    console.time(`Lookup ${email}`);
    const lookupData = await fastlane.lookupCustomerByEmail(email);
    console.timeEnd(`Lookup ${email}`);
    console.log(lookupData);

    await fastlane.mountWatermark('.googlepay-field');
})();
